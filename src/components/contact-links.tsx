...
import { StaggerChildren, StaggerItem } from "@/components/animate-in";
    ...
    <>
      <StaggerChildren className="space-y-4" staggerDelay={0.08}>
        {contactItems.map(({ icon: Icon, label, value, href }) => (
          <StaggerItem key={label}>
            <Link
              href={href}
              className="line-card flex items-center gap-4 hover:border-[var(--forest)] transition group"
              onClick={() => posthog.capture("contact_info_clicked", { method: label.toLowerCase() })}
            >
      ...
      {company.whatsappUrl && (
        <Link
          ...
          onClick={() => posthog.capture("whatsapp_clicked", { source: "contact_page" })}
        >
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-[#166534]">Message pré-rempli · Réponse rapide</p>
          </div>
